...
class SeatAdjusterApp(VehicleApp):
    ...
    async def on_set_position_request_received(self, data_str: str) -> None:
        ...
        logger.debug(
            "Seat Position Request %s",
            data_str
        )

        await self.Vehicle.Cabin.Seat.Row1.Pos1.Position.set(int(data_str))

        seat_position = ( 
            await
            self.Vehicle.Cabin.Seat.Row1.Pos1.Position.get()
            ).value

        await self.publish_event(
            SET_POSITION_RESPONSE_TOPIC,
            str(seat_position)
        )
